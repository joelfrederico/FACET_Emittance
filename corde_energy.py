...
def Energy_Axis_ELANEX(dataset, QS):
    ...
    if dataset == 13437 or dataset == 13438:
        y0    = np.float64(1589.0) # pixel position of E0 (20.35 GeV).
        eta_0 = np.float64(949.72) # nominal dipole dispersion in pixel, corresponding to 59.5 mm.
        
    elif dataset == 13448 or dataset == 13449:
        y0    = np.float64(1605.5 - 0.7923*(20.35+QS)   ) # y0 is adjusted to account for QS dispersion.
        eta_0 = np.float64(949.72 + (20.35+QS) * 0.7923 ) # added QS dispersion of 0.7923 pix per QS GeV.
            
    elif dataset == 13450:
        y0    = np.float64(1655 - 3.321*(20.35+QS)     ) # y0 is adjusted to account for QS dispersion.
        eta_0 = np.float64(949.72 + (20.35+QS) * 3.321 ) # added QS dispersion of 3.321 pix per QS GeV.
        
    elif dataset == 13537:
        y0    = np.float64(576 + 0.5193*(20.35+QS)     ) # y0 is adjusted to account for QS dispersion.
        eta_0 = np.float64(49.72 - (20.35+QS) * 0.5193 ) # added QS dispersion of -0.5193 pix per QS GeV.
    else:
        logger.error('This dataset does not have a calibrated energy axis. Returning the nominal energy axis.')
        y0    = np.float64(1590   ) # pixel position of E0 (20.35 GeV).
        eta_0 = np.float64(949.72 ) # nominal dipole dispersion in pixel, corresponding to 59.5 mm.
    ...
    y      = np.linspace(-6000,6000,num=12001,dtype=np.float64)
    E0     = np.float64(20.35)
    E_FULL = E0 / ( np.float64(1) - y/eta_0 )
    
    delta_y = np.floor(np.float64(55)*QS/(QS+E0) / (np.float64(1e-3) * cal_ELANEX))
    
    i1_compare = np.round(np.float64(1)-y0+delta_y) 
    i1 = np.where( y== i1_compare )[0][0]
    i2_compare = np.round(np.float64(734)-y0+delta_y)
...
def Energy_Axis_CMOS_FAR(dataset, QS):
    ...
    y  = np.linspace(1,2559,2559,dtype=np.float64)
    E0 = np.float64(20.35)
